...
1. Clone the repo and go to this sample:
   ...
   cd rezzy-samples/samples/bulk-generate/python
   ```

2. Install dependencies with uv (no separate venv step needed):
...
- **Resumes only:** `uv run generate --mode resume`
- **Cover letters only:** `uv run generate --mode cover-letter`
- **Both:** `uv run generate --mode both`
